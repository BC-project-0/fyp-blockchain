...
def random_value_gen(node):
    ...
    num = choices(['true', 'false'], [node.probability,
                  50 - node.probability], k=5).count('true')
    ans = num / 5
    if ans < 0.7:
        return False
    else:
...
def heartbeat(currNode):
    while not currNode.published:
        ...
        event, data = "Heartbeat", "Heartbeat from leader -> " + \
            str(currNode.host) + ":" + str(currNode.port)
        currNode.send_encrypted_msg(event, data)
        time.sleep(3)
...
def publish_block(currNode):
    ...
    currNode.blockchain.add_block(Block(latest_block.index + 1, str(currNode.host) + ":" + str(
        currNode.port), latest_block.hash, data, signature, pool_data, [], int(time.time())))
